...
module Part1 = struct
  let run s =
    ...
    let ingredient_is_fresh ingr =
      List.exists fresh_ranges ~f:(fun (start, end_) -> start <= ingr && ingr <= end_)
    in
...
module Part2 = struct
  let run s =
    ...
    let rec merge_intervals = function
      | ([] | [ _ ]) as xs -> xs
      | (a, b) :: (c, d) :: xs when b >= c -> merge_intervals ((min a c, max b d) :: xs)
      | x :: xs -> x :: merge_intervals xs
    in
    let merged_ranges = merge_intervals fresh_ranges in
    List.sum (module Int) merged_ranges ~f:(fun (start, end_) -> end_ - start + 1)
  ;;
end
