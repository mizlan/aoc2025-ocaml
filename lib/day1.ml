...
let parse s =
  let parse_line line =
    let dir =
      match line.[0] with 'L' -> -1 | 'R' -> 1 | _ -> failwith "bad"
    in
...
module Part1 = struct
  let run s =
    ...
    in
    let acc, _ =
      List.fold_left rotations ~init:(0 (* acc *), 50 (* phase *))
  ...
  let part2 rotations =
    let num_zeros, _ =
      List.fold_left rotations ~init:(0, 50) ~f:(fun (acc, phase) rotation ->
          ...
          in
          if additional_zeros > 0 then
            print_s [%message "adding" (additional_zeros : int)];
